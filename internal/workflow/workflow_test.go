...
func TestRunClarifyInvalidJSON(t *testing.T) {
	...
	mock := newMockRunner()
	mock.runFunc = func(ctx context.Context, p string, outputCh chan<- runner.OutputLine) error {
		return os.WriteFile(filepath.Join(tmpDir, questionsFile), []byte("not json"), 0644)
	}
...
func TestRunClarifyEmptyArray(t *testing.T) {
	...
	mock := newMockRunner()
	mock.runFunc = func(ctx context.Context, p string, outputCh chan<- runner.OutputLine) error {
		return os.WriteFile(filepath.Join(tmpDir, questionsFile), []byte("[]"), 0644)
	}
...
func TestRunClarifyWithQuestions(t *testing.T) {
	...
	mock.runFunc = func(ctx context.Context, p string, outputCh chan<- runner.OutputLine) error {
		...
		return os.WriteFile(filepath.Join(tmpDir, questionsFile), []byte(data), 0644)
	}
...
func TestRunClarifyQuestionsFileCleanedUp(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(tmpDir, questionsFile)); !os.IsNotExist(err) {
		t.Error("questions file should be deleted after RunClarify")
	}
}

// TestAllEventIsEventMethodsIncludesClarifying ensures EventClarifyingQuestions

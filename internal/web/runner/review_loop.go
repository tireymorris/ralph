...
func (r *registryReviewLoop) Snapshot() (iteration int, fingerprint string, elapsedMs int64) {
	run, ok := r.registry.Get(r.runID)
	if !ok {
		return 0, "", 0
	}
	return run.ReviewIteration, run.ReviewFingerprint, run.ReviewElapsedMs
}

...
func TestFollowUpForwardsRunnerOutput(t *testing.T) {
	...
	prdPath := filepath.Join(workDir, "prd.json")
	prdData := `{"version":1,"project_name":"Test","stories":[{"id":"s1","title":"Story","description":"Do it","slices":[{"id":"slice-1","behavior":"AC","red_hint":"add failing test","passes":false}],"priority":1,"passes":false}]}`
